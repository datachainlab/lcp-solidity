...
library IbcLightclientsLcpV1ClientState {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      if (fieldId == 11) {
        pointer += _read_zkdcap_verifier_info(pointer, bs, r);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
   ...
   * @return The number of bytes decoded
   */
  function _read_zkdcap_verifier_info(
    uint256 p,
    bytes memory bs,
    Data memory r
  ) internal pure returns (uint) {
    (bytes memory x, uint256 sz) = ProtoBufRuntime._decode_bytes(p, bs);
    r.zkdcap_verifier_info = x;
    return sz;
  }
    ...
    returns (uint)
  {
    ...
    if (r.zkdcap_verifier_info.length != 0) {
    pointer += ProtoBufRuntime._encode_key(
      11,
      ProtoBufRuntime.WireType.LengthDelim,
      pointer,
    ...
    pointer += ProtoBufRuntime._encode_bytes(r.zkdcap_verifier_info, pointer, bs);
    }
    return pointer - offset;
  }
  // nested encoder
  ...
  function _estimate(
    ...
    e += 1 + ProtoBufRuntime._sz_lendelim(r.zkdcap_verifier_info.length);
    return e;
  }
  // empty checker
  ...
  function store(Data memory input, Data storage output) internal {
    ...
    output.zkdcap_verifier_info = input.zkdcap_verifier_info;
